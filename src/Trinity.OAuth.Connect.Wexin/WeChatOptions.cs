...
namespace Trinity.OAuth.Connect.Wexin
{
    public class WeChatOptions : OAuthOptions
    {
        ...
        public static string UserBaseScope = "snsapi_base";
        ...
        public WeChatOptions()
        {
            ...
            {
                var privilege = new List<string>();
                if (user.TryGetProperty("privilege", out var property))
                {
                    privilege = property.EnumerateArray().Select(x => x.GetString()).ToList();
                }

                return string.Join(",", privilege);
            });
            // ClaimActions.MapCustomJson("urn:wechat:privilege", user =>  string.Join(",",user.GetProperty("privilege")?.EnumerateArray()?.Select(s => s.ToString()).ToArray() ?? new string[0]));
